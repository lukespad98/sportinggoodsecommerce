...
interface Props {
  searchParams: {
    ...
    category?: string
    size?: string
    search?: StringDecoder
    name?: string
  }
}

export default async function Page({searchParams}: Props) {
  const {price, color, category, size, search, name} = searchParams
  const priceOrder = searchParams.price ?  `| order(price ${price})` : ""
  const nameOrder = searchParams.name ? `| order(name ${name})` : ""
  const order = `${priceOrder}${nameOrder}`
